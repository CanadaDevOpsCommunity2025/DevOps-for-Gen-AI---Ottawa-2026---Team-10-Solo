...
> **Implementation status:** the spec below is now backed by a working
> reference implementation. See `AGENT_MANDATE.md` for the policy this
> platform enforces on AI-agent-built apps, and the Quickstart below to run
> it. `sentinel-rag-observability.html` (this directory) is the original
> static mockup with hardcoded sample data — the real, database-backed
> dashboard now lives in `platform/public/`.
...
```bash
# 1. Platform (backend API + dashboard) — http://localhost:4000
cd platform && npm install && npm run seed && npm start

# 2. Flight Recorder — traffic generator + its own console, http://localhost:8000
cd flight-recorder
python3 -m venv .venv && ./.venv/bin/pip install -r requirements.txt
./.venv/bin/python harness/run.py --backfill 6h --backfill-count 900 --backfill-only
./.venv/bin/python server.py &                       # its own console, port 8000
./.venv/bin/python register_with_sentinel.py         # once — onboard its apps into Sentinel
./.venv/bin/python harness/run.py --rate 3 --scenario mixed --sink sqlite,sentinel
```

Open `http://localhost:4000` and sign in as `admin@sentinel.platform` /
`admin123` (platform admin), `compliance@sentinel.platform` /
`compliance123` (compliance officer), or `jae.kim@northwind.com` /
`password123` (owner of the Northwind RAG app — tenant-scoped view only).

Watch the Overview and Audit Trail fill with real signal generated by
flight-recorder's traffic: `undeclared_pii_detected` (declared PII was only
`["EMAIL", "CREDIT_CARD"]`; the generator also plants SINs, DOBs, phone
numbers), plus `leakage_risk`, `ungrounded_generation`, `agent_loop`, and the
rest of flight-recorder's own RCA verdicts, all routed through Sentinel's
alerting.

Then open `http://localhost:8000` for the other half of the demo — click
into a `retrieval miss` or `tool loop` trace for the span-level waterfall, or
press the `PII leak` inject button to watch a new failure land in real time.
See `flight-recorder/START.md` for a scripted walkthrough.

There are deliberately **two dashboards**, not one:

- **Sentinel** (`platform/`, port 4000) — the product's observability
  dashboard: PII compliance drift, retention/region policy, access logs,
  audit trails, tenant isolation. This is what an AI-administrator looks at.
- **Flight Recorder console** (`flight-recorder/`, port 8000) — a synthetic
  RAG-traffic generator with its own span-level waterfall/RCA console, for
  generating realistic test data and rehearsing failure modes. See
  `flight-recorder/START.md`.

They stay separate UIs, but `flight-recorder`'s generated traces also feed
Sentinel (see "Bridge to Sentinel" in `flight-recorder/README.md`) via
`python3 flight-recorder/register_with_sentinel.py` and
`--sink sqlite,sentinel` — so the same synthetic traffic shows up as real
PII-drift alerts and RCA-verdict audit events on the Sentinel dashboard, not
just as a waterfall.
...
| Path | What it is |
|---|---|
| `platform/` | Express + SQLite backend and dashboard — the real implementation of everything below |
| `sdk/` | `@sentinel/observe-sdk` — the drop-in client a future Node app built by an AI agent would import |
| `flight-recorder/` | Synthetic RAG-trace generator + its own console; the current live demo, bridged into Sentinel via a dedicated sink |
| `deploy/` | AWS deployment scripts — provisions one EC2 instance running both services under systemd |
| `AGENT_MANDATE.md` | Instructions fed to an AI coding agent so it wires in observability regardless of what the user's prompt said |
